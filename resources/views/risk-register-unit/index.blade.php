...
function submitRequestEdit() {
    ...
    Swal.fire({
        ...
        if(result.isConfirmed) {
            ...
            $.ajax({
                url: '{{ route("risk-register-unit.submit-request-edit") }}',
                type: 'POST',
...
function reopenRiskRegisterUnit(id, riskName = 'risiko ini') {
    const url = "{{ route('risk-register-unit.reopen', ['riskRegister' => ':id']) }}".replace(':id', id);
