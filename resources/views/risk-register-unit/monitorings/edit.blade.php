...
<style>
    ...
    #modalUpdateKri input[name="batas_aman"],
    #modalUpdateKri input[name="batas_waspada"],
    #modalUpdateKri input[name="batas_bahaya"] {
        max-width: 100%;
        overflow-wrap: anywhere;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            Swal.fire({
                ...
                window.location.href = '{!! route('risk-register-unit.monitorings.index', ['period' => request()->route('period'), 'unit_id' => $risk?->unit_id, 'quarter' => $quarter, 'month' => $month]) !!}';
            });
        },
        error: function(xhr) {
...
$(document).ready(function() {
    ...
    $('.btn-action').on('click', function() {
        ...
        if (action === 'save' || action === 'save-and-close') {
            ...
            // Tampilkan & Populasikan field tambahan jika format baru (Danantara)
            // if (isNewFormat) {
            //     $('.kri-new-fields').removeClass('d-none');
                
            //     // Populate Ambang Batas dengan satuan
            //     let satuan = kriProject.satuan_kri ? ' ' + kriProject.satuan_kri : '';
            //     $('#modalUpdateKri input[name="kri_risk_limit"]').val((kriProject.risk_limit ? Number(kriProject.risk_limit).toLocaleString('id-ID') : '-') + satuan);
            //     $('#modalUpdateKri input[name="kri_risk_appetite"]').val((kriProject.risk_appetite ? Number(kriProject.risk_appetite).toLocaleString('id-ID') : '-') + satuan);
            //     $('#modalUpdateKri input[name="kri_risk_tolerance"]').val((kriProject.risk_tolerance ? Number(kriProject.risk_tolerance).toLocaleString('id-ID') : '-') + satuan);
            // } else {
