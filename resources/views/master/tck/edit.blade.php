...
<div class="row justify-content-center">
  <div class="col-12 col-md-10 col-lg-7">
    <form class="card" method="POST" action="{{ route('tck.update', $tck) }}">
      @csrf
      @method('PUT')
      <div class="card-header d-flex justify-content-between">
        <div class="d-block col-10">
          <h2 class="h3 mb-2">Edit Target Capaian Kinerja</h2>
          <span class="ff-heading-med">{{ $tck->title }}</span>
        </div>
        ...
        <div class="row gx-0 gy-3">
          <div class="form-group d-md-flex">
            ...
            <select class="form-select js-select-hide-search" name="periode_id" required>
              ...
              <option value="{{ $id }}" {{ $id == $tck->periode_id ? 'selected' : '' }}>{{ $tahun }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group d-md-flex">
            ...
            <select class="form-select js-select-hide-search" name="unit_id" required>
              ...
              <option value="{{ $id }}" {{ $id == $tck->unit_id ? 'selected' : '' }}>{{ $name }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group d-md-flex">
            ...
            <input class="form-control" id="title" name="title" type="text"
              placeholder="Masukkan Target Capaian Kinerja" value="{{ $tck->title }}" />
          </div>
        </div>
      </div>
