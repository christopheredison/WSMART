    ...
    <div class="card">
      <div class="card-header">
        <div class="d-flex align-items-center gap-3">
          ...
          <h2 class="h3">Jenis Risiko</h2>
          <div id="bulk-select-replace-element" class="col-auto ms-auto">
            <a href="{{ route('jenis-risiko.create') }}" class="btn btn-outline-info btn-sm">
        ...
        <table class="table table-bulk-select table-hover dataTable" data-filter="true" data-paging="true">
          <thead>
            <tr>
              ...
              <th class="sort mw-10r" data-sort="kategori">Kategori Risiko</th>
              <th class="sort mw-10r" data-sort="title">Jenis Risiko</th>
              <th class="no-sort white-space-nowrap" data-sort="action">Action</th>
            </tr>
          </thead>
          <tbody class="list" id="bulk-select-body">
            @foreach ($jenisRisiko as $index => $item)
            <tr>
              ...
              <td class="title">{{ $item->kategori->title }}</td>
              ...
              <td class="white-space-nowrap">
                ...
                $formAction = route('jenis-risiko.restore', $item->id);
                @endphp
                @include('partials.modal-restore-alert')
                @else
                <a href="{{ route('jenis-risiko.edit', $item) }}" class="btn-input-icon" data-bs-toggle="tooltip"
                  title="Edit">
                ...
                $itemId = $item->id;
                $innerItemText = $item->title;
                $formAction = route('jenis-risiko.destroy', $item->id);
                @endphp
