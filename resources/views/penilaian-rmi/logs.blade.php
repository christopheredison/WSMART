    ...
    <div class="card">
      <div class="card-header">
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <h2 class="h4 mb-0">Log Perubahan Penilaian RMI</h2>
            <small class="text-muted">Riwayat perubahan data periode, aspek dimensi, dan aspek kinerja</small>
          </div>
          <div class="ms-auto">
            <a href="{{ route('penilaian-rmi.index') }}" class="btn btn-outline-secondary btn-sm">
              <span class="bx bx-arrow-back"></span>
              <span class="ms-1">Kembali ke Periode RMI</span>
            </a>
          </div>
        </div>
      </div>
      <div class="card-body">
        <form method="GET" action="{{ route('penilaian-rmi.logs') }}" class="row g-2 align-items-end mb-3">
          <div class="col-md-5 col-lg-4">
            <label for="period_id" class="form-label">Periode</label>
            <select name="period_id" id="period_id" class="form-select">
              <option value="">Semua Periode</option>
              @foreach ($periods as $period)
                <option value="{{ $period->id }}" {{ (string) $periodId === (string) $period->id ? 'selected' : '' }}>
                  {{ $period->year }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-md-4 col-lg-3">
            <label for="event" class="form-label">Filter Event</label>
            <select name="event" id="event" class="form-select">
              <option value="">Semua Event</option>
              <option value="created" {{ $event === 'created' ? 'selected' : '' }}>Ditambahkan</option>
              <option value="updated" {{ $event === 'updated' ? 'selected' : '' }}>Diperbarui</option>
              <option value="deleted" {{ $event === 'deleted' ? 'selected' : '' }}>Dihapus</option>
              <option value="restored" {{ $event === 'restored' ? 'selected' : '' }}>Dipulihkan</option>
            </select>
          </div>
          <div class="col-auto">
            ...
            <tbody>
              @forelse ($audits as $index => $audit)
                @php
                  ...
                  $periodYear = data_get($audit, 'rmiPeriod.year') ?? '-';
              ...
              @empty
                <tr>
                  <td colspan="7" class="text-center py-4 text-muted">
                    Belum ada log perubahan untuk penilaian RMI.
                  </td>
                </tr>
              @endforelse
            </tbody>
