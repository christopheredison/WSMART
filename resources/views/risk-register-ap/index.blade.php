                  ...
                  <div class="d-flex flex-column align-items-center gap-1">
                    ...
                    @if($item->request_edit == 1)
                      <a href="javascript:void(0)"
                         ...
                         onclick='approveRequestEdit({{ $item->id }}, @json($item->request_edit_reason), @json($item->peristiwa_risiko), @json($item->unit->name ?? "Anak Perusahaan Tidak Diketahui"))'
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content p-0">
      ...
      <div class="modal-body">
        ...
        <div class="alert alert-info mb-3">
          Risiko ini telah di-Publish. Silakan ajukan request jika perlu melakukan perubahan. Request akan dikirimkan ke <strong>{{ \App\Models\IdentifikasiRisiko::getRequestEditVerifierLabel('ap') }}</strong>.
        </div>
...
function submitRequestEdit() {
    ...
    Swal.fire({
        title: 'Kirim Request?',
        text: 'Request edit risiko akan dikirimkan ke {{ \App\Models\IdentifikasiRisiko::getRequestEditVerifierLabel('ap') }}.',
        icon: 'question',
        showCancelButton: true,
...
const canVerifyRequestEditAp = {{ \App\Models\IdentifikasiRisiko::canVerifyRequestEdit(auth()->user(), 'ap') ? 'true' : 'false' }};

function approveRequestEdit(id, reason, riskName, unitName) {
    reason = reason || 'Tidak ada informasi alasan.';
    riskName = riskName || 'Risiko Anak Perusahaan';
    unitName = unitName || 'Anak Perusahaan Tidak Diketahui';

    if (!canVerifyRequestEditAp) {
        Swal.fire({
            title: 'Detail Request Edit',
            icon: 'info',
            html: `<div class="text-start">
                <p class="mb-2">Risiko: <strong>${riskName}</strong></p>
                <p class="mb-2">Anak Perusahaan: <strong>${unitName}</strong></p>
    ...
    Swal.fire({
        ...
        html: `Apakah Anda ingin menyetujui atau menolak request edit untuk risiko <strong>${riskName}</strong> pada anak perusahaan <strong>${unitName}</strong> ini?<br><br>` +
...
const canVerifyCloseAp = {{ \App\Models\IdentifikasiRisiko::canVerifyRequestEdit(auth()->user(), 'ap') ? 'true' : 'false' }};
const approveCloseUrl = '{{ route("risk-register-ap.approve-close-request") }}';
const rejectCloseUrl = '{{ route("risk-register-ap.reject-close-request") }}';
const apRiskDetailUrlTemplate = '{{ route("risk-register-ap.view", ["riskRegister" => "__RISK_ID__"]) }}';
...
function verifyCloseRequest(id, reason, riskName, requestedBy, requestedAt) {
    ...
    const detailUrl = apRiskDetailUrlTemplate.replace('__RISK_ID__', id);
    ...
    if ($autoVerifyCloseIdFromQuery) {
        ...
        if ($riskToCloseAuto && (int) $riskToCloseAuto->close_request === \App\Models\IdentifikasiRisiko::CLOSE_REQUEST_PENDING) {
            $autoVerifyClosePreset = [
                ...
                'riskName' => $riskToCloseAuto->peristiwa_risiko ?? 'Risiko Anak Perusahaan',
