...
use App\Models\PenyebabRisikoApLed;
use App\Models\PerlakuanPenyebabRisikoApLed;
...
class ApLEDController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row) {
                    return view('ap-led._table_action', compact('row'))->render();
                })
    ...
    public function create($periode)
    {
        ...
        return view('ap-led.create', compact('periodes', 'kategoriKejadians', 'jenisRisikos', 'jabatans', 'periode', 'unitId'));
    }
        ...
        try {
            ...
            if (is_array($penyebabData)) {
                foreach ($penyebabData as $penyebab) {
                    $ledPenyebab = PenyebabRisikoApLed::create([
                        'loss_event_ap_id' => $led->id,
            ...
            if ($request->input('create_risk_from_led') == '1') {
                ...
                DB::commit();
                return redirect()->route('risk-register-ap.edit', ['riskRegister' => $newUnitRisk->id])
                    ->with('success', 'Loss Event berhasil dibuat dan Divisi Risk baru telah ditambahkan.');
            }

            // Jika user memilih "Tidak", cukup simpan LED
            DB::commit();
            return redirect()->route('ap-led.index-by-periode', ['periode' => $request->periode_id])
    ...
    {
        $lossEvent = LossEventAp::with('penyebabRisikoLeds.perlakuanPenyebabRisiko')->findOrFail($id);
        $periodes = Periode::orderBy('tahun', 'desc')->with('identifikasiRisikos.penyebabRisikos.perlakuanPenyebabRisiko')->get();
        $kategoriKejadians = KategoriKejadian::all();
        $jenisRisikos = JenisRisiko::with('kategoriRisiko')->get();
        $jabatans = Jabatan::all();
        ...
        return view('ap-led.edit', compact(
          'lossEvent',
          'periodes',
          'kategoriKejadians',
          'jenisRisikos',
    ...
    {
        // dd($request->all());
        $lossEvent = LossEventAp::findOrFail($id);
        ...
        try {
            ...
            foreach ($penyebabDataFromRequest as $penyebabItem) {
                ...
                $penyebabData = [
                    'loss_event_ap_id' => $lossEvent->id,
                    'penyebab_risiko' => $penyebabItem['penyebab_risiko'],
                ];

                if ($isNewPenyebab) {
                    $penyebabRecord = PenyebabRisikoApLed::create($penyebabData);
                } else {
                    $penyebabRecord = PenyebabRisikoApLed::find($penyebabItem['id']);
                    if ($penyebabRecord) {
                    ...
                    if (!empty($penyebabItem['perlakuan']) && is_array($penyebabItem['perlakuan'])) {
                        ...
                        foreach ($penyebabItem['perlakuan'] as $perlakuanItem) {
                            ...
                            if ($isNewPerlakuan) {
                                ...
                                $perlakuanRecord = PerlakuanPenyebabRisikoApLed::find($perlakuanItem['id']);
                                if ($perlakuanRecord) {
                        ...
                        if (!empty($perlakuanToDelete)) {
                            PerlakuanPenyebabRisikoApLed::destroy($perlakuanToDelete);
                        }
            ...
            if (!empty($penyebabToDelete)) {
              PenyebabRisikoApLed::destroy($penyebabToDelete);
            }
    ...
    public function destroy($id)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Data Loss Event Anak Perusahaan berhasil dihapus'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menghapus data Loss Event Anak Perusahaan'
            ], 500);
        }
    }
    ...
    {
        $lossEvent = LossEventAp::with(['kategoriKejadian', 'kategoriRisiko', 'jenisRisiko'])->findOrFail($id);
        
        return view('ap-led.show', compact('lossEvent'));
    }
    ...
    public function riskChangeToLed(IdentifikasiRisiko $riskRegister)
    {
        ...
        return view('ap-led.change-to-led', compact(
            'risiko',
            'kategoriKejadians',
            'jenisRisikos',
            'analisa',
        ...
        try {
            ...
            if (is_array($penyebabData)) {
                foreach ($penyebabData as $penyebabItem) {
                    $newLedPenyebab = PenyebabRisikoApLed::create([
                        'loss_event_ap_id' => $led->id,
                        'penyebab_risiko' => $penyebabItem['penyebab_risiko'],
                    ]);
                    ...
                    if (!empty($penyebabItem['perlakuan']) && is_array($penyebabItem['perlakuan'])) {
                        foreach ($penyebabItem['perlakuan'] as $perlakuanItem) {
                            ...
                            PerlakuanPenyebabRisikoApLed::create([
            ...
            return redirect()->route('risk-register-ap.monitorings.index', ['period' => $riskRegister->periode_id])
